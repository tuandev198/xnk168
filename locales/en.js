
const en = {
  greeting: "Xin chào",
  language: "Ngôn ngữ",
  switch_to_en: "Chuyển sang Tiếng Anh",
  product_catalog: "Danh mục sản phẩm",
  categories: {
    que: "QUẾ",
    ot: "ỚT HIỂM",
    hoahoi: "BÔNG HỒI",
    thaoqua: "THẢO QUẢ",
    sa: "SẢ",
    lachuoi: "LÁ CHUỐI",
    mia: "MÍA",
    bun: "BÚN",
    banhtrang: "BÁNH TRÁNG",
    chanhday: "CHANH DÂY",
  },
  nav: {
  home: "TRANG CHỦ",
  products: "SẢN PHẨM",
  contact: "LIÊN HỆ",
  about: "VỀ CHÚNG TÔI",
  feedback: "GÓP Ý",
  submenu: {
    dry: "HÀNG KHÔ",
    frozen: "HÀNG ĐÔNG LẠNH",
    household: "ĐỒ GIA DỤNG",
    clean: "THỰC PHẨM ĂN LIỀN",
    send: "GỬI PHẢN HỒI",
    consult: "TƯ VẤN KINH DOANH",
    story: "CÂU CHUYỆN THƯƠNG HIỆU",
    cert: "CHỨNG NHẬN",
    award: "GIẢI THƯỞNG",
    intro: "GIỚI THIỆU DOANH NGHIỆP",
  },
},

  whyChooseUs: {
    title: "Tại sao chọn chúng tôi?",
    description: "Với những nghệ nhân tận tâm và nhân viên tài năng cùng những câu chuyện làm bánh đầy cảm hứng, Binh Vinh là lựa chọn dành cho những người đam mê hương vị món ăn ngon.",
    benefits: [
      {
        title: "Giá cả phải chăng",
        desc: "Cam kết cung cấp sản phẩm có nguồn gốc rõ ràng và kiểm soát chất lượng.",
      },
      {
        title: "Hương vị tinh tế",
        desc: "Nguyên liệu được lựa chọn kỹ càng ở giai đoạn tươi ngon nhất.",
      },
      {
        title: "Sản phẩm chất lượng cao",
        desc: "Đảm bảo nguồn gốc sản phẩm được kiểm soát nghiêm ngặt và truy xuất được.",
      },
    ],
  },
   bestProducts: {
    title: "Sản phẩm bán chạy",
    description: "Pellentesque massa placerat duis ultricies lacus sit sed.",
    categories: ["Hạt khô", "Gia vị cay"],
    saleLabel: "Giảm Giá",
    products: [
      { name: "Sả", price: 197, oldPrice: 300, sale: true, imageUrl: '/images/sa.jpg', link:"/product/sa-cay-frozen-chopped-lemoongass"},
  { name: "Lá Chuối", price: 969, oldPrice: 1000, sale: true , imageUrl: '/images/lachuoi.jpg', link:"/product/la-chuoi-tuoi-banana-leal" },
 
  { name: "Bún", price: 197, oldPrice: 300, sale: true , imageUrl: '/images/bun.jpg', link:"/product/bun-bo-hue-rice-vermiceui"},
  { name: "Bánh Tráng", price: 197, oldPrice: 300, sale: true , imageUrl: '/images/banhtrang.jpg', link:"/product/banh-trang-cha-gio"},
  { name: "Nước Chanh Dây", price: 197, oldPrice: 300, sale: true , imageUrl: '/images/chanhday.jpg' , link:"/product/nuoc-chanh-day-frozen-pure-passion-fruit-juice"},
  { name: "Thảo Quả", price: 199, oldPrice: 405, sale: true, imageUrl: '/images/QUE.jpg', link:"/shop?shop=driedfoods"},
  { name: "Ớt", price: 289, sale: false, imageUrl: '/images/OT.jpg', link:"/shop?shop=driedfoods" },
  { name: "Hoa Hồi", price: 289, sale: false , imageUrl: '/images/THAOQUA.jpg', link:"/shop?shop=driedfoods"},
  
   { name: "Mía", price: 289, sale: false, imageUrl: '/images/mia.jpg', link:"/shop?shop=driedfoods"},
]


  },
};


export default en;
